...
erDiagram
    subgraph student_db
        STUDENT {
            string id PK "Student ID (e.g., S1001)"
            string name
            string email
            string programme
            double gpa
    ...
    subgraph enrolment_db
        ENROLMENT {
            bigint id PK
            string studentId FK "Logical reference to STUDENT.id"
            string courseCode FK "Logical reference to COURSE_CAPACITY.courseCode"
            string semester
            string status "CONFIRMED / PROVISIONAL"
        ...
        CACHED_STUDENT {
            string id PK "Cached student ID"
            string name
            string programme
        }
    end
    ...
    subgraph notification_db
        NOTIFICATION_RECORD {
            ...
            string eventId
            string eventType
            string recipient FK "Logical reference to STUDENT.id"
    ...
    STUDENT ||--o{ ENROLMENT : "enrols in (Logical)"
    STUDENT ||--o{ BOOK_LOAN : "borrows (Logical)"
    STUDENT ||--o{ ROOM_BOOKING : "reserves (Logical)"
    STUDENT ||--o{ NOTIFICATION_RECORD : "receives (Logical)"
    COURSE_CAPACITY ||--o{ ENROLMENT : "allocates"
```
